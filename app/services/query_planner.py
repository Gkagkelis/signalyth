...
def drop_bare_topic_route(queries: list[str], topic: str, market: str) -> list[str]:
    ...
    for q in queries:
        ...
        if bare == subject:
            continue
        kept.append(q)
    # Returning an empty list is intentional: source adapters decide
    # whether a native market control makes the bare subject safe. Facebook and
    # Instagram must not silently restore the global firehose.
    return kept
...
def make_generic_source_plan(source: str, target: int, draft: AnalysisDraft, queries: list[str], cfg: dict, source_budget: float) -> SourcePlan:
    ...
    if semantic == "urls":
        ...
        if not tags:
            # A generic Instagram Actor has no semantic_broad_probe to fall
            # back on, and the batching fallback below would otherwise stuff
            # the RAW TOPIC STRING into a URL field — a paid call with garbage
            # input. The global topic tag is the one valid last resort here.
            tags = [hashtag(draft.topic)]
        discovery_values = [f"https://www.instagram.com/explore/tags/{t.lower()}/" for t in tags]
    else:
...
def make_source_plan(source: str, target: int, draft: AnalysisDraft, queries: list[str], registry: dict, source_budget: float) -> SourcePlan:
    ...
    if source in NATIVE_MARKET_FILTER_SOURCES:
        ...
        if not primary:
            # Empty keywords must still be a valid brief — for EVERY source
            # that lost its bare route, not only Facebook. Without this, a
            # topic-only X brief planned zero subruns and the source silently
            # collected nothing. The market routes generated by build_terms
            # become the first diversified wave.
            promoted = topups[:3]
            primary = promoted
    ...
    if source == "x":
        ...
        tags = instagram_discovery_tags(draft)
        # On a market run, an empty safe-tag list is a valid state. Do not
        # invent #NikeGreece and do not restore global #Nike as primary.
        # The bounded semantic_broad_probe below is the controlled fallback.
        if not tags and not str(draft.market or "").strip():
            tags = [hashtag(routes["topic"])]
        if tags:
            primary_tag = tags[0]
        ...
        fallback_queries = (
            [f'{routes["topic"]} Greece', f'{routes["topic"]} Ελλάδα', f'{routes["topic"]} Ellada']
            if draft.market.casefold() == "greece"
            else [routes["topic"]]
        )
        pqueries = uniq([_facebook_safe_query(x) for x in primary if x]) or [
            _facebook_safe_query(x) for x in fallback_queries
        ]
    ...
    if str(draft.market or "").strip() and source in {"facebook", "instagram"}:
        ...
        if cap_items > 0:
            ...
            preview.append({
                "source": source,
                "route": "semantic_broad_probe",
                "query": routes["topic"] if source == "facebook" else f"#{hashtag(routes['topic'])}",
                "order": len(preview) + 1,
                "editable": False,
                "date_from": draft.date_from.isoformat(),
                "date_to": draft.date_to.isoformat(),
                "strategy": draft.search_strategy,
            })
    return SourcePlan(
...
def build_collection_plan(draft: AnalysisDraft) -> CollectionPlan:
    ...
    return CollectionPlan(client=draft.client,topic=draft.topic,market=draft.market,date_from=draft.date_from,date_to=draft.date_to,report_language=draft.report_language,research_type=draft.research_type,media_handling=draft.media_handling,owned_accounts=list(draft.owned_accounts or []),media_accounts=list(draft.media_accounts or []),client_logo=draft.client_logo,brand_accent=draft.brand_accent,
