    ...
    if settings.signalyth_ai_enabled and settings.openai_api_key:
        try:
            ...
            response=client.responses.create(model=settings.signalyth_ai_reasoning_model,store=False,
                instructions=(("You write as a senior political communications analyst; frame findings in terms of political narratives, coalitions of opinion and reputational risk. " if str(plan.get("research_type") or "market")=="political" else "You write as a senior market/brand analyst; frame findings in terms of consumer perception, brand equity and commercial risk. ")+f"You are the senior analyst synthesis layer of SIGNALYTH. Write in {lang}. Use ONLY the supplied deterministic indicators, investigations and evidence references. "
